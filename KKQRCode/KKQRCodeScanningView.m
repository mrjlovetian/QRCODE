...
@interface KKQRCodeScanningView ()
...
@implementation KKQRCodeScanningView
...
#pragma mark - - - 扫描边角imageView的创建
    ...
    UIImage *left_image = [UIImage imageNamed:@"KKQRCode.bundle/KKCodeLeftTop"];
    ...
    // 右上侧的image
    UIImage *right_image = [UIImage imageNamed:@"KKQRCode.bundle/KKCodeRightTop"];
    ...
    // 左下侧的image
    UIImage *left_image_down = [UIImage imageNamed:@"KKQRCode.bundle/KKCodeLeftBottom"];
    ...
    // 右下侧的image
    UIImage *right_image_down = [UIImage imageNamed:@"KKQRCode.bundle/KKCodeRightBottom"];
...
- (UIImageView *)scanningline {
    if (!_scanningline) {
        ...
        _scanningline.image = [UIImage imageNamed:@"KKQRCode.bundle/KKCodeScanningLine"];
...
- (void)addTimer {
    ...
    self.timer = [NSTimer scheduledTimerWithTimeInterval:KKQRCodeScanningLineAnimation target:self selector:@selector(timeAction) userInfo:nil repeats:YES];
...
- (void)timeAction {
    ...
    if (flag) {
        ...
        flag = NO;
        [UIView animateWithDuration:KKQRCodeScanningLineAnimation animations:^{
            frame.origin.y += 5;
